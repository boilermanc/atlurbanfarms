-- Fix: RPC function was setting order status to 'paid' or 'pending' which
-- violates orders_status_check constraint. Use the status provided by the
-- frontend (which correctly maps to valid values), with a safe fallback.
