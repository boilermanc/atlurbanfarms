-- Add Purchase Order support to orders table
...
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS po_status TEXT
    CHECK (po_status IN ('pending_verification', 'verified', 'invoiced', 'paid', 'cancelled'));

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS po_verified_at TIMESTAMPTZ;

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS po_verified_by UUID REFERENCES public.customers(id);

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS po_invoiced_at TIMESTAMPTZ;

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS po_paid_at TIMESTAMPTZ;

-- Indexes for admin PO management queries
CREATE INDEX IF NOT EXISTS idx_orders_po_status
  ON public.orders (po_status)
  WHERE po_status IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_orders_payment_method_po
  ON public.orders (payment_method)
  WHERE payment_method = 'purchase_order';

-- Update the create_order_with_inventory_check RPC to include PO fields
