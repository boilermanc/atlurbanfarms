-- Fix: The RPC from 20260330 regressed the pickup_reservations INSERT.
-- Column name is "location_id" (not "pickup_location_id") and "schedule_id" was dropped.
-- This caused "column pickup_location_id does not exist" errors when creating pickup orders.
-- See: https://github.com/boilermanc/atlurbanfarms/issues/67
...
BEGIN
  ...
  -- FIXED: Use correct column names "location_id" and "schedule_id"
  -- (not "pickup_location_id" which doesn't exist in pickup_reservations table)
