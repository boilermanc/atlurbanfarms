-- Add growing_system column to orders table for customer's growing system selection
-- and update the order creation RPC to persist it.

ALTER TABLE orders ADD COLUMN IF NOT EXISTS growing_system TEXT;

-- Recreate the RPC to include growing_system in the INSERT
