-- Allow admins to backdate manual orders (e.g. Square greenhouse sales
-- entered after the fact for the Sales Tax Report).
--
-- The existing create_order_with_inventory_check RPC accepts paid_at from
-- the caller (since the 2026-05-11 fix) but does not accept created_at,
-- so a backdated order's created_at always defaults to NOW() and the
-- Weekly Sales Report groups the row under the entry date, not the sale date.
--
-- This migration redefines the RPC with one addition: created_at is added to
-- the INSERT column list and VALUES clause, defaulting to NOW() when the
-- caller does not provide one (so the storefront checkout path, which does
-- not send created_at, is unaffected). All other columns/behavior are a
-- verbatim copy of the 2026-05-11 manual-fix version.
...
BEGIN
  ...
  INSERT INTO orders (
    ...
    payment_method, payment_status, paid_at, created_at, created_by_admin_id,
    ...
    CASE
      ...
      ELSE NULL
    END,
    -- created_at: honor caller-supplied value to support admin backdating;
    -- default to NOW() when absent so customer checkout (which never sends
