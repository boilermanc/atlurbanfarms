-- Pending orders table: stores checkout data between payment intent creation
-- and order creation. Enables webhook-based order creation as a fallback
-- if the browser closes after payment succeeds.
...
CREATE INDEX IF NOT EXISTS idx_pending_orders_pi
  ON pending_orders(stripe_payment_intent_id);

ALTER TABLE pending_orders ENABLE ROW LEVEL SECURITY;
-- No RLS policies: accessed only by edge functions via service role


-- Update the create_order_with_inventory_check RPC to accept stripe_payment_intent_id
-- so both the client and webhook can set it at order creation time.
