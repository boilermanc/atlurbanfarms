...
import { X, Trash2, Check, Loader2, Minus } from 'lucide-react';
import { AdminRole } from '../hooks/useAdminUsers';
import { ALL_PERMISSIONS } from '../pages/AdminUsersPage';

interface RolePermissionsModalProps {
  role: AdminRole | null;
  onSave: (role: AdminRole) => Promise<{ success: boolean; error?: string }> | void;
  onDelete?: (roleId: string) => Promise<{ success: boolean; error?: string }>;
  onClose: () => void;
}
...
const RolePermissionsModal: React.FC<RolePermissionsModalProps> = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await onSave(updatedRole);
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-180px)] space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-slate-50"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 resize-none disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-slate-50"
            />
          </div>
              ...
              {Object.entries(permissionsByCategory).map(([category, categoryPermissions]) => (
                <div key={category} className="bg-slate-50 rounded-xl border border-slate-200 overflow-hidden">
                  ...
                  <button
                    ...
                    disabled={isSystemRole}
                    className="w-full px-4 py-3 flex items-center justify-between hover:bg-slate-100 transition-colors disabled:cursor-default"
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${
                        ...
                        {isCategoryPartiallySelected(category) && !isCategoryFullySelected(category) && (
                    ...
                    {categoryPermissions.map(permission => (
                      <label
                        ...
                        className={`flex items-center gap-3 p-2 rounded-lg transition-colors ${
                          isSystemRole ? 'cursor-default' : 'cursor-pointer hover:bg-white'
                        }`}
                      >
                        <input
                          type="checkbox"
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <span className="ml-2 text-slate-400 text-xs font-mono">{permission.key}</span>
                        </div>
                      </label>
                    ))}
                  </div>
                ...
                <div className="p-4 bg-red-50 border border-red-200 rounded-xl">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className="flex-1 px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-xl font-medium transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {saving ? (
                        <>
